...
def parish():
    import shutil
    print(f'Update Complete! You are now on version {System.Cache.System.Version}!')
...
def dl():
    ...
    try:
        os.system('git clone https://github.com/smoke-wolf/GitHub-Package-Manager.git')  # Download Updated Version
    except:
        print('Network may have timed out- Please ensure you are connected to the internet')
...
def Update():
    AllContents_Downloaded = os.listdir(f'{SourceDirectory}System/Cache/System/Update/GitHub-Package-Manager')
    AllContents = os.listdir(SourceDirectory)

    try:
        ver2 = open(
            f'{SourceDirectory}System/Cache/System/Update/GitHub-Package-Manager/System/Cache/System/Version.py', 'r')
        ver4 = ver2.read()
        print(f'''
    ...
    for Attempt in AllContents:
        ...
        if Attempt in AllContents_Downloaded:
            ...
            try:
                ...
                Update = open(f'{SourceDirectory}System/Cache/System/Update/GitHub-Package-Manager/{Attempt}', 'r')
                ...
                for Attempt_i in SourceFolder:
                    if Attempt_i in UpdateFolder:
                        ...
                        if Attempt_i == 'index':
                            ...
                            try:
                                open(f'{SourceDirectory}{Attempt}/{Attempt_i}', 'w').close()

                                SourceClean = open(f'{SourceDirectory}{Attempt}/{Attempt_i}', 'a')
                                ...
                                SourceFolder_i = os.listdir(f'{SourceDirectory}{Attempt}/{Attempt_i}')
                                ...
                                for Attempt_ii in SourceFolder_i:
                                    if Attempt_ii in UpdateFolder_i:
                                        ...
                                        try:
                                            open(f'{SourceDirectory}{Attempt}/{Attempt_i}/{Attempt_ii}', 'w').close()

                                            SourceClean = open(f'{SourceDirectory}{Attempt}/{Attempt_i}/{Attempt_ii}', 'a')
                                            Update = open(
                                            ...
                                            for Attempt_iii in SourceFolder_ii:
                                                if Attempt_iii in UpdateFolder_ii:
                                                    print(f'Common Found {Attempt_iii}')
                                                    try:
                                                        open(
...
try:
    ...
    try:
        ...
        if str(CurrentVersion) in str(vrl):
            ...
            print(f'Version {CurrentVersion}, is either already updated or is not recent enough to be compatible')
