...
def Password(Event, Input):
    ...
    if uuidToken in DefaultTkn:
        ...
        EV.NewEvent(event='Same Device As Created On = False! ->> WARNING Separate Device', Pol=0)
    ...
    if UserID == User.UserProfile.UserPrivileges:
        ...
        EV.NewEvent(event='Same User As Configured = False! ->> WARNING Not Original User', Pol=0)
...
def LAUNCH(Password):
    ...
    if Password in User.UserProfile.UserKey:
        decrypt(f'{User.UserProfile.SourceDirectory}System/Cache/System/ErrorLog/Event.db')
...
def write_key():
    ...
    import User
    with open(f"{User.UserProfile.SourceDirectory}/User/UserKey.key", "wb") as key_file:
        key_file.write(key)

    Crypt(key, Password=input('Please Enter Your Password To Add Encryption: '))
...
def main(message, password, f, address):
    ...
    if f == 2:
        import User.UserProfile
        addy = f'{User.UserProfile.SourceDirectory}System/Cache/User/ADDRESSLOG'
        fr = open(addy, 'r')
        print('==============')
        line = fr.read()
        print(line)
