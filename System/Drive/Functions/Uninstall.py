...
def All():
    ...
    if inp == 2:
        ...
        with open(f'{cwd}/System/Cache/System/Local/Int.txt', 'r') as r, open(f'{cwd}/System/Cache/System/Local/Int2.txt','w') as o:
            for line in r:
                if line.strip():
                    o.write(line)
        f = open(f'{cwd}/System/Cache/System/Local/Int2.txt', "r")
            ...
            if count1 == count:
                ...
                try:
                    try:
                        ...
                        if len(listOfWords) > 0:
                            v = listOfWords[1]
                            EV.NewEvent(event=f'Attempting [!]{v[1:-1]}[!]', Pol=0)
                            import shutil
                            shutil.rmtree(v[1:-1])
                            EV.NewEvent(event=f'Directory Removed [!]{v[1:-1]}[!]', Pol=0)
                            print('Project Removed')
                    except:
                        import System.Drive.Errors_Events.EventMan as EV
                        # Remove all characters before the character '-' from string
