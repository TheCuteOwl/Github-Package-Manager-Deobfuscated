...
def Main():
    ...
    if args[1] == '-I':  # Intalling through Git takes 2 arguments
        ...
        try:
            ...
            EV.NewEvent(event=f'{cwd}/System/Cache/System/GitHub/Downloads : Created --COMMAND LINE', Pol=0)
        except:
            pass
        Source = cwd
        print(spacer)
        ...
        try:  # downloading from GitHub
            ...
            for x in Files1:
                if x in Files:
                    ...
                    EV.NewEvent(event=f'Package Downloaded: {x} --COMMAND LINE', Pol=0)
            ...
            for y in SDir1:
                if y in SDir:
                    ...
                    if fi == '0':
                        ...
                        In2 = input('Install command (2) leave black for none: ')
                        In3 = input('Install command (3) leave black for none: ')
                        ...
                        try:
                            ...
                            EV.NewEvent(event=f'Requirements Downloaded With: {Req}--COMMAND LINE', Pol=0)

                        except:
                            import System.Drive.Errors_Events.EventMan as EV

                            EV.NewEvent(event=f'Requirements did not download--COMMAND LINE', Pol=0)
                        print('Install: [2/2]')
                        ...
                        if ffi == 'py':
                            ...
                            EV.NewEvent(event=f'Launch Command Created: {Form} --COMMAND LINE', Pol=0)
                            os.chdir(Source)
                            cwdd = os.getcwd()
        ...
        EV.NewEvent(event=f'Launch Command Complete: {Form} --COMMAND LINE', Pol=0)
    ...
    ====================================================''')
        ...
        for line in lines:
            ...
            if count1 == count:
                ...
                try:
                    ...
                    EV.NewEvent(event=f'Failure: {value1} --COMMAND LINE', Pol=0)
                ...
                try:
                    ...
                    EV.NewEvent(event=f'Directory changed [!]{value1}[!] --COMMAND LINE', Pol=0)
                    ...
                    EV.NewEvent(event=f'Dir Change Canceled --COMMAND LINE', Pol=0)
                ...
                try:
                    ...
                    EV.NewEvent(event=f'Command Executed [!]{value}[!] --COMMAND LINE', Pol=0)
                except:
        ...
        for line in lines:
            ...
            if count1 == count:
                ...
                EV.NewEvent(event=f'Change Directory: {cc}  --COMMAND LINE', Pol=0)
                try:
                    os.chdir(cc)
                    import System.Drive.Errors_Events.EventMan as EV

                    EV.NewEvent(event=f'DirChange = Success! --COMMAND LINE', Pol=0)
                    EV.NewEvent(event=f'os executed command [!]{value}[!] --COMMAND LINE', Pol=0)
                    try:
                        os.system(f'{value}')
                        sys.exit(0)
                    except:
                    ...
                    EV.NewEvent(event=f'DirChange = Failed --COMMAND LINE', Pol=0)
        ...
        for line in lines:
            ...
            if count1 == count:
                ...
                try:
                    for line in lines:
                        try:
                            ...
                            EV.NewEvent(event=f'Command {line} --COMMAND LINE', Pol=0)
                            os.system(line)
                        except:
                            import System.Drive.Errors_Events.EventMan as EV

                            EV.NewEvent(event=f'Command failed --COMMAND LINE', Pol=0)
                            exit(0)
                ...
                try:
                    ...
                    EV.NewEvent(event=f'os executed command [!]{A}[!] --COMMAND LINE', Pol=0)
                    sys.exit(0)
                except:
    ...
    #   Uninstalling
        ...
        if valuee > count:
            ...
            try:
                ...
                EV.NewEvent(event=f'DirRemoval = Success! --COMMAND LINE', Pol=0)
            ...
            import System.Drive.Errors_Events.EventMan as EV
            EV.NewEvent(event=f'Removing Directory: {cc} --COMMAND LINE', Pol=0)
            try:
                import System.Drive.Errors_Events.EventMan as EV
                import shutil
        ...
        with open(f'{cwd}/System/Cache/System/Local/Int.txt', 'r') as r, open(
                f'{cwd}/System/Cache/System/Local/Int2.txt', 'w') as o:
            for line in r:
                if line.strip():
                    o.write(line)
        f = open(f'{cwd}/System/Cache/System/Local/Int2.txt', "r")
        ...
        for line in lines:
            ...
            if count1 == count:
                ...
                with open(f'{cwd}/System/Cache/System/Local/Int.txt', 'r') as file:
                    lines55 = file.readlines()
                try:
                    with open(f'{cwd}/System/Cache/System/Local/Int.txt', 'w') as file:
                        for lineff in lines55:
                            if lineff.find(value) != -1:
                                ...
                                file.write(lineff)
                    import System.Drive.Errors_Events.EventMan as EV
                    EV.NewEvent(event=f'Launch Command Removed --COMMAND LINE', Pol=0)

                except:
                    import System.Drive.Errors_Events.EventMan as EV
                    EV.NewEvent(event=f'Launch Command Failed To Remove --COMMAND LINE', Pol=0)
                ...
                try:
                    try:
                        ...
                        if len(listOfWords) > 0:
                            v = listOfWords[1]
                            EV.NewEvent(event=f'Attempting [!]{v[1:-1]}[!] --COMMAND LINE', Pol=0)
                            import shutil
                            shutil.rmtree(v[1:-1])
                            EV.NewEvent(event=f'Directory Removed [!]{v[1:-1]}[!] --COMMAND LINE', Pol=0)
                            print('Project Removed')
                    except:
                        import System.Drive.Errors_Events.EventMan as EV
                        # Remove all characters before the character '-' from string
                        ...
                        if len(listOfWords) > 0:
                            v = listOfWords[1]
                            EV.NewEvent(event=f'Attempting [!]{v[:-4]}[!] --COMMAND LINE', Pol=0)
                            import shutil
                            shutil.rmtree(v[:-1])
                            shutil.rmtree(v[:-4])
                            EV.NewEvent(event=f'Directory Removed [!]{v[:-4]}[!] --COMMAND LINE', Pol=0)
                            print('Project Removed')
                except:
                    print('Project Failed To Remove')
                    import System.Drive.Errors_Events.EventMan as EV
                    EV.NewEvent(event=f'Dir Removal Canceled --COMMAND LINE', Pol=0)
